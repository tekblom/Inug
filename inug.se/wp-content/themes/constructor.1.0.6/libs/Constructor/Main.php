...
class Constructor_Main extends Constructor_Abstract
{
    ...
    function getSlideshow($in = false)
    {
        ...
        if (!$in) {
            $width = (int)($this->_options['layout']['width'] - 36);
        } else {            
            // switch statement for $this->_options['sidebar']
            switch ($this->_options['sidebar']) {
                case 'none':
                    $width = (int)($this->_options['layout']['width'] - 16);
                    break;
                case 'two':
                case 'two-right':
                case 'two-left':
    ...
    function getDefaultSlideshow($width, $height) 
    {
		...
		if(qtrans_getLanguage()=="es") {
			$options['slideshow'] = get_option('home').'/?theme-constructor=slideshow_spanska&w='.$width.'&h='.$height;
		}
		else
		{
